...
#'   \item{isgood:}          Boolean indicating the exit status of the function.
#'   \item{msgs:}            Messages to be passed back to the user.
#'   \item{apl_res:}         Results from FM_app_preload() if run, \code{NULL} otherwise. 
#'   \item{ss_res:}          Results from ASM_save_state() if run, \code{NULL} otherwise. 
#'   \item{ls_res:}          Results from ASM_load_state() if run, \code{NULL} otherwise. 
...
#' 
#'res = FM_test_preload(sources=sources, test_save_state = FALSE)
#'
#'res$isgood
FM_test_preload = function(
  sources         = NULL, 
  preload_files   = NULL, 
  preload_dir     = tempdir(),
