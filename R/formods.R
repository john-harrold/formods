...
FM_tc = function(cmd, tc_env, capture){
  ...
  tcres = tryCatch({
    ...
    # If capture hasn't been specified then we capture everything that has been
    # generated. We get all objects then remove those defined above or as inputs
    if(is.null(capture)){
      capture = setdiff(ls(),  c("cmd", "tc_env", "capture", "name"))
    }
    # Capturing objects
    FMTC_obj_cap = list()
    for(FMTC_obj_name in capture){
      FMTC_obj_cap[[FMTC_obj_name]] = get(FMTC_obj_name)
    }
    list(capture = FMTC_obj_cap, isgood=TRUE)},
    error = function(e) {
